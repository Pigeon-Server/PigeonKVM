  ...
  <div class="toolsBar">
   ...
   <v-table>
    ...
    <tbody>
      ...
      >
        <td>{{ item.id }}</td>
        <td>{{ item.name }}<v-icon icon="mdi:mdi-square-edit-outline" size="x-small" @click="action('rename', item.id)"></v-icon></td>
        <td>{{ item.creator ? item.creator : "未知"}}</td>
        <td>{{ item.createdAt ? item.createdAt : "未知" }}</td>
        <td>{{ item.disable ? "已禁用" : "已启用" }}<v-icon icon="mdi:mdi-square-edit-outline" size="x-small" @click="action('update_status', item.id)"></v-icon></td>
        <td>
...
<!--    新建权限组-->
    ...
    >
      <v-card>
        ...
        <v-card-text>
          <v-text-field type="text" label="请输入要创建的权限组名" v-model="newPermissionGroupDialog.newGroupName"></v-text-field>
          ...
          <v-card class="pa-1">
            ...
            <v-card-actions>
              <v-btn color="warning" :disabled="newPermissionGroupDialog.selected.length < 1" @click="newPermissionGroupDialog.selected = []">清空已选择</v-btn>
            </v-card-actions>
          </v-card>
        </v-card-text>
        <v-card-actions>
    ...
    >
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="success" @click="updateStatus(updateGroupStatusDialog.gid,updateGroupStatusDialog.value)">确定</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
<!--    修改权限组信息-->
