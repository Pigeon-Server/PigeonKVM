  ...
  <div id="fileList">
    ...
    <v-table fixed-header hover>
      ...
      >
        ...
        <td class="action">
          <v-btn class="openEdit" :disabled="item.fileType === '文件夹'" size="small"
                 @click="fileAction(item.fileName, 'edit')">编辑
          </v-btn>
          <v-btn class="reName" size="small" @click="fileAction(item.fileName, 'reName')">重命名</v-btn>
          <v-btn class="download" size="small" :disabled="item.fileType === '文件夹'"
                 @click="fileAction(item.fileName, 'download')">下载
          </v-btn>
  ...
  <div id="dialogs">
    ...
    >
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error"
                 @click="dialogs.reName.flag = false;dialogs.reName.input = null;dialogs.reName.filename = null">取消
          </v-btn>
    ...
    >
      <v-card>
        ...
        <v-card-text id="edit">
          <ace-edit :value="dialogs.edit.value" :language="dialogs.edit.language"
                    @edit-event="(data)=>{this.dialogs.edit.change = data}"></ace-edit>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    >
      <v-card>
        ...
        <v-card-text>
          <v-text-field v-model="dialogs.inputDialog.input" :label="dialogs.inputDialog.label"
                        :type="dialogs.inputDialog.type"></v-text-field>
        </v-card-text>
        <v-card-actions>
          <v-btn color="error"
                 @click="dialogs.inputDialog.cancelCallback ? dialogs.inputDialog.cancelCallback() : this.dialogs.inputDialog.flag = false">
            取消
          </v-btn>
          <v-btn color="success"
                 @click="dialogs.inputDialog.confirmationCallback(this.dialogs.inputDialog.input);this.dialogs.inputDialog.flag = false">
            确定
          </v-btn>
        </v-card-actions>
      </v-card>
