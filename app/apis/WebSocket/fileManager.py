...
class fileManagerPageWebSocket(WebsocketConsumer):
    ...
    def connect(self):
        ...
        if user:
            ...
            writeAccessLog(self.__userID, self.__clientIP, "FileManager Websocket Connect")
        else:
            self.close(-1)
    ...
    def disconnect(self, close_code):
        ...
        writeAccessLog(self.__userID, self.__clientIP, f"FileManager Websocket Disconnect(Code:{close_code})")
        raise StopConsumer
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        if bytes_data:
            ...
            flag = fileHeader[-31:]
            if flag == "   IPKVM-Core_FileUploadData   ".encode():
                fileData = bytes_data[63:]
                Log.debug("收到一个文件包")
                Log.debug("fileHash:" + str(fileHash))
                FT.receiveFiles(fileData, fileHash.decode())
                writeFileChangeLog(self.__userID, "Upload File Chunk(Bytes)", f"temp/{fileHash}")
                ...
                match action:
                    ...
                    case "newDir":
                        ...
                        if path and dirName:
                            ...
                            if not os.path.exists(os.path.join(targetPath, dirName)):
                                ...
                                writeFileChangeLog(self.__userID, "New Dir", os.path.join(targetPath, dirName))
                            else:
                    ...
                    case "newFile":
                        ...
                        if path and fileName:
                            ...
                            if FT.createFile(os.path.join(targetPath, fileName)):
                                self.sendJson(FT.getFileList(self.__homePath, path))
                                writeFileChangeLog(self.__userID, "New File", os.path.join(targetPath, fileName))
                    ...
                    case "reName":
                        ...
                        if oldFileName and newFileName:
                            ...
                            if os.path.exists(old) and not os.path.exists(new):
                                os.rename(old, new)
                                self.sendJson(FT.getFileList(self.__homePath, path))
                                writeFileChangeLog(self.__userID, "ReName", f"{old}-->{new}")
                            ...
                            if isinstance(fileName, list):
                                for item in fileName:
                                    if FT.delFile(os.path.join(targetPath, item)):
                                        ...
                                        writeFileChangeLog(self.__userID, "Del File", os.path.join(targetPath, item))
                                    else:
                                        self.sendErrorMessage("目录或文件删除失败")
                                        Log.debug("文件或目录已删除")
                            else:
                                if FT.delFile(os.path.join(targetPath, fileName)):
                                    writeFileChangeLog(self.__userID, "Del File", os.path.join(targetPath, fileName))
                                    self.sendSuccessMessage("目录或文件删除成功")
                                else:
                                    self.sendErrorMessage("目录或文件删除失败")
                                    Log.debug("文件或目录已删除")
                    ...
                    case "uploadFile":
                        ...
                        if path and chunkHash and base64:
                            ...
                            writeFileChangeLog(self.__userID, "Upload File Chunk(Base64)", f"temp/{chunkHash}")
                        else:
                    ...
                    case "mergeFile":
                        ...
                        if path and filename and fileHash and chunksHash:
                            ...
                            self.sendJson(FT.getFileList(self.__homePath, path))
                            writeFileChangeLog(self.__userID, "Merge File", os.path.join(FT.mergePaths(self.__homePath, path), filename))
                        else:
                            self.sendErrorMessage("[上传]参数不完整")
                            Log.warning("参数不完整")
                    # 新建临时链接
                    case "newTemporaryLink":
                        filename = data.get("filename")
                        if path and filename:
                            User_Model = Users.objects.filter(id=self.__userID).first()
                            ...
                            writeFileChangeLog(self.__userID, "New Temporary Link", os.path.join(FT.mergePaths(self.__homePath, path), filename))
                        else:
                        ...
                        if filename and not self.__currentEditor:
                            ...
                            if os.path.exists(self.__currentEditor) and os.path.getsize(
                                ...
                                with open(self.__currentEditor, "r",
                                    ...
                                    writeFileChangeLog(self.__userID, "Start Edit File", self.__currentEditor)
                    ...
                    # 取消编辑文件
                    case "cancelEdit":
                        writeFileChangeLog(self.__userID, "Cancel Edit File", self.__currentEditor)
                        self.__currentEditor = None
                    ...
                    case "saveFile":
                        ...
                        if value and self.__currentEditor:
                            ...
                            writeFileChangeLog(self.__userID, "Save Edit File", self.__currentEditor)
