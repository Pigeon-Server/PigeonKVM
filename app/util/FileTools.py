...
def move(user_id, homePath, sourceFileDirectory, selectFiles, toDirectory):
    ...
    for item in selectFiles:
        ...
        writeFileChangeLog(user_id, "Move File", os.path.join(sourceFileDirectory, item)+"-->"+toDirectory)
...
def copy(user_id, homePath, sourceFileDirectory, selectFiles, toDirectory):
    ...
    for item in selectFiles:
        ...
        if os.path.isfile(os.path.join(sourceFileDirectory, item)):
            ...
            writeFileChangeLog(user_id, "Copy File", os.path.join(sourceFileDirectory, item)+"-->"+toDirectory)
            ...
            writeFileChangeLog(user_id, "Copy Dir", os.path.join(sourceFileDirectory, item) + "-->" + toDirectory)
...
def compression(user_id, homePath, path, files, output, callback=None):
    ...
    for filename in files:
        if not os.path.exists(os.path.join(inputFilePath, filename)):
            ...
            writeFileChangeLog(user_id, "Compression File", os.path.join(inputFilePath,  filename)+"-->"+outputFilePath)
...
def decompress(user_id, homePath, path, files, outputPath=None, callback=None):
    ...
    for archiveName in files:
        ...
        writeFileChangeLog(user_id, "Decompress File", os.path.join(inputFilePath, archiveName)+"-->"+outputPath)
        if callback:
            callback()